// HELLO IF YOU'RE READING THIS, YOU'RE NOT SUPPOSED TO BUT LIKE, ENJOY MIND NUMBING CODE!
// if you wanna copy stuff from this, great im honoured :)
// too lazy to do better
...
// nav data
const BIG_BOXES = [
  { symbol: "▲", label: "About Me",  glitch: "Not you."      },
  { symbol: "▼", label: "Contact",   glitch: "Don't bother." },
  { symbol: "◆", label: "Portfolio", glitch: "Just projects." },
  { symbol: "⬤", label: "Status",    glitch: "Right now."    }
];

const SMALL_BOXES = [
  { symbol: "⏹", label: "Home",      glitch: "Escape.",        target: showBigBoxes    },
  { symbol: "▲", label: "About Me",  glitch: "Not you.",       target: showParagraphs  },
  { symbol: "▼", label: "Contact",   glitch: "Don't bother.",  target: showContactInfo },
  { symbol: "◆", label: "Portfolio", glitch: "Just projects.", target: showPortfolio   },
...
function showBigBoxes() {
  ...
  BIG_BOXES.forEach((box, i) => {
    const { boxDiv } = createBox(
      box,
      { width: "200px", height: "200px", borderRadius: "10px", transition: "opacity 2s ease, transform 0.2s ease" },
      { fontSize: "4rem" },
      { fontSize: "1.8rem" }
    );
...
function showParagraphs() {
  ...
  const paraEls   = paras.map(t   => appendAnimatedPara(t, "clamp(0.9rem,2.5vw,1.1rem)"));
  const talentEls = talents.map(t => appendAnimatedPara(t, "clamp(0.85rem,2.2vw,1rem)"));

  paraEls.forEach((p, i)   => setTimeout(() => revealEl(p),  i * 1000 + 500));

  const talentStart = paras.length * 1000 + 500;
  talentEls.forEach((p, i) => setTimeout(() => revealEl(p), talentStart + i * 750));

  const totalTime = talentStart + talents.length * 750;
  setTimeout(() => showFourSmallBoxes(showParagraphs, container), totalTime + 500);
}
...
function showContactInfo() {
  ...
  const contacts = [
    ...
    { text: "Instagram: @MiloSelfEsteem",  clickable: true,  href: "https://instagram.com/Milo_SelfEsteem"        },
    { text: "Discord: @45g",               clickable: false, isDiscord: true, copyTarget: "45g"                   },
    { text: "Twitter/X: @MiloSeIfEsteem",  clickable: true,  href: "https://twitter.com/MiloSeIfEsteem"           },
    { text: "Github: @MiloSelfEsteem",     clickable: true,  href: "https://github.com/MiloSelfEsteem"            }
  ];
  ...
  // nav timing from array length
  setTimeout(() => showFourSmallBoxes(showContactInfo, container), navDelay);
}

function showPortfolio() {
  const { container } = createSection("Portfolio", { maxWidth: "960px", width: "100%" });
  ...
  const projects = [
    {
      ...
      skill: "PineScript",
      description: "A collection of custom PineScript indicators developed for financial analysis, ranging from higher timeframe candle overlays to near-automated trading strategies. Built through a combination of reverse-engineering existing solutions and original development."
    },
    {
      title: "MSE XAUUSD ORB — Expert Advisor",
      skill: "MQL5 (C++ derivative)",
      description: "An automated trading bot built for MetaTrader 5, designed for fully objective, rule-based trade execution on XAUUSD. Backtesting across a five-year window yielded an ~8,000% return."
    },
    {
      title: "AscendUpward — Team Tool",
      skill: "AutoHotkey (AHK)",
      description: "A real-time communication tool developed for Guild Wars 2 World vs World operations, enabling near-instant relay of tactical instructions to groups of 50+ players during high-intensity engagements."
    },
    {
      title: "Mic-Ro-Phone — Social Game",
      skill: "LUA",
      description: "A voice-chat-centric social game developed in two weeks for the Roblox platform. Players could socialise, compete in mini-games, and explore an open world environment."
    },
    {
      title: "L???",
      skill: "Javascript / Python",
      description: "An optimized fork of a prominent open-source repository, refactored to enhance execution speed, performance and utility for a global user base."
    },
    {
      title: "Temporary Placeholder",
      skill: "Node.js / CSS",
      description: "Insert some text here about a random project that is being worked on. This is a very important project but it has to be very secretive. Something something latin words."
    }
  ];
  ...
  const prevBtn       = makeSpan(`[ ◄ ]`, { fontSize: "1.2rem", transition: "color 0.3s ease" });
  const pageIndicator = makeSpan(`PAGE ${currentPage + 1} / ${totalPages}`, { fontSize: "0.9rem", letterSpacing: "0.1em" });
  const nextBtn       = makeSpan(`[ ► ]`, { fontSize: "1.2rem", transition: "color 0.3s ease" });
  ...
  function renderPage(isInitialLoad = true) {
    ...
    css(prevBtn, { opacity: atStart ? "0.3" : "1", cursor: atStart ? "default" : "pointer" });
    css(nextBtn, { opacity: atEnd   ? "0.3" : "1", cursor: atEnd   ? "default" : "pointer" });
    ...
    currentSlice.forEach((proj, i) => {
      ...
      css(desc, { fontSize: "clamp(0.85rem, 1.6vw, 0.95rem)", lineHeight: "1.6", color: "rgba(255,255,255,0.75)" });
  ...
  setTimeout(() => showFourSmallBoxes(showPortfolio, container), initialAnimationTime + 1000);
}
...
function showGallery() {
  ...
  // Dev toggle: set to a string to override, or null for live schedule
  ...
  if (!document.getElementById("liveDotStyle")) {
    ...
    style.textContent = "@keyframes liveDot { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.3; transform: scale(0.6); } }";
  ...
  function makeBox(sectionLabel, value, isLive) {
    ...
    css(top, { display: "flex", alignItems: "center", justifyContent: "space-between" });
  ...
  const musicBox    = makeBox("Listening", "Establishing secure connection...", false);
...
function fetchRecentTrack(callback) {
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      if (!track) { callback("No recent tracks found.", false); return; }
...
function startSubtleGlitch(el, a, b, minMs = 1000, maxMs = 3000) {
  ...
  function glitch() {
    ...
    setTimeout(glitch, Math.random() * (maxMs - minMs) + minMs);
  }
  setTimeout(glitch, Math.random() * (maxMs - minMs) + minMs);
}
      ...
      setTimeout(() => {
        backspace(MSG1.length, () => {
          typeMessage(MSG2, () => {
            ...
            startSubtleGlitch(TEXT_EL, "How are you?", "Click me.", 50, 2500);
          });
        });
      }, PAUSE_AFTER1);
    });
...
document.addEventListener("mouseover", e => {
  const isClickable = e.target.closest('a, [style*="cursor: pointer"], .clickable-image');
  if (isClickable) {
    customCursor.style.setProperty("width",            "45px",        "important");
    customCursor.style.setProperty("height",           "45px",        "important");
    customCursor.style.setProperty("background-color", "transparent", "important");
...
document.addEventListener("mousedown", () => {
  customCursor.style.setProperty("transform", "translate(-50%, -50%) scale(0.6)", "important");
});
document.addEventListener("mouseup", () => {
  customCursor.style.setProperty("transform", "translate(-50%, -50%) scale(1)", "important");
});

// background effects

function animateParticlesEffect() {
  const COUNT = 40, TRAIL = 17;
  const particles = Array.from({ length: COUNT }, () => {
    const angle = Math.random() * Math.PI * 2, speed = Math.random() + 0.5;
  ...
  (function animate() {
    ...
    particles.forEach(p => {
      ...
      if (Math.random() < 0.001 && !p.glitch) p.glitch = 5 + Math.floor(Math.random() * 5);
...
function animateLinesEffect() {
  ...
  (function animate() {
    ...
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    lines.forEach(l => {
      ctx.strokeStyle = `rgba(255,255,255,${Math.max(0.05, Math.min(0.6, l.shade + Math.sin(t + l.phase) * 0.05))})`;
      ctx.beginPath();
      ctx.moveTo(l.x, l.y);
...
function animateNoiseCloudEffect() {
  ...
  (function animate() {
    ...
    clouds.forEach(c => {
      ctx.globalAlpha = Math.max(0.3, 0.5 + Math.sin(performance.now() * 0.001 + c.phase) * 0.2);
...
function animateDigitalRainEffect() {
  ...
  const drops = new Int16Array(cols).fill(1);
  const words = ["ARTIFICIALINTELLIGENCE", "MILOSELFESTEEM", "MILO", "WHOAREYOU", "WELCOMETOTHEWEBSITE", "AI"];
  const active = Array.from({ length: cols }, () => words[Math.floor(Math.random() * words.length)]);
  let frame = 0, rafId;
  ...
  (function animate() {
    ...
    if (frame++ % 3 === 0) {
      for (let i = 0; i < drops.length; i++) {
        ...
        ctx.fillText(active[i][drops[i] % active[i].length], i * fs, drops[i] * fs);
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    ...
    // Re-run the same effect index isn't stored, so we just pick a new random one on return
